...
Это нужно, чтобы понять, какой урок искать дальше."""
    ...
    МЫ НЕ ИЩЕМ КОНКРЕТНЫЙ НОМЕР. МЫ ИЩЕМ ПЕРВУЮ ТОЧКУ В БУДУЩЕМ.

    -------------------------------------------------
    1. ПОРЯДОК ИМЕЕТ ЗНАЧЕНИЕ
    -------------------------------------------------
    LESSON_TIMES — это словарь, но цикл 'for lesson in lessons' 
    идет по урокам в строгом порядке (1, 2, 3...). 
    Это гарантирует, что мы встретим 10:50 РАНЬШЕ, чем 11:40.

    -------------------------------------------------
    2. ПОЧЕМУ ЭТО РАБОТАЕТ? (Пример 10:12)
    -------------------------------------------------
    Текущее время: "10:12"
    
    - Проверка Урока №3 (Start: "09:50"):
      "09:50" > "10:12" --> False. (Это прошлое).
    
    - Проверка Урока №4 (Start: "10:50"):
      "10:50" > "10:12" --> True! (Это будущее).

    -------------------------------------------------
    3. ТОЧКА ВЫХОДА (Мгновенный возврат)
    -------------------------------------------------
    Как только мы нашли "10:50", функция делает 'return'.
    Всё, что было ПОСЛЕ (Уроки 5, 6, 7...), программа даже не видит. 
    Она "выплевывает" 4-й урок как результат.
    
    -------------------------------------------------
    4. А ЧТО ЕСЛИ УРОК ИДЕТ СЕЙЧАС?
    -------------------------------------------------
    Если сейчас 11:00 (урок №4 начался в 10:50):
    - "10:50" > "11:00" --> False.
    - Бот пойдет к Уроку №5 (11:40).
    - "11:40" > "11:00" --> True.
    - Итог: Бот скажет, что следующий урок — Пятый.

    -------------------------------------------------
    ПРИМЕНЕНИЕ:
    Это классический алгоритм поиска в отсортированном списке. 
    Мы не сравниваем "между чем и чем" находится время. 
    Мы просто ищем первую точку, которая больше текущей.
    """
    welcome_text = (
        f"🏠 *Главное меню*\n\n"
...
"""
    ЛОГИЧЕСКИЙ ГЕНЕРАТОР КОНТЕКСТА (CONTEXT ENGINE):
    ===============================================

    ЗАЧЕМ ЭТО НУЖНО:
    Чтобы бот не был "тупым" списком кнопок. Это создает эффект живого 
    ассистента, который знает, что сейчас происходит у ученика. 

    ПОЧЕМУ ЭТО РАБОТАЕТ ИМЕННО ТАК:
    Мы используем динамическое наращивание строки (накопление данных). 
    Вместо того чтобы создавать три разных шаблона сообщения, мы создаем 
    один "скелет" и наращиваем на него "мясо" в зависимости от условий.

    -------------------------------------------------
    1. АТОМАРНЫЙ РАЗБОР СЦЕНАРИЕВ:
    -------------------------------------------------
    А) СЦЕНАРИЙ "БУДУЩЕЕ": (if next_lesson)
       Если наш поисковик нашел урок, время которого > текущего.
       КАК: Мы используем f-строку и оператор +=. 
       РЕЗУЛЬТАТ: К тексту приклеивается название предмета и время.

    Б) СЦЕНАРИЙ "ОТДЫХ" (now.isoweekday() > 5):
       Если уроков нет, бот проверяет: "А какой сегодня день?".
       ЛОГИКА: 1-5 это будни, 6-7 (Суббота, Воскресенье) — это > 5.
       РЕЗУЛЬТАТ: Бот радостно сообщает о выходных.

    В) СЦЕНАРИЙ "КОНЕЦ ДНЯ" (else):
       Если уроков нет и это будний день (пн-пт).
       ЛОГИКА: Значит, все 8 уроков уже прошли.
       РЕЗУЛЬТАТ: Бот пишет "Уроков больше нет" (т.е. иди отдыхай, бро).

    -------------------------------------------------
    2. КАК ЭТО РАБОТАЕТ "ПОД КАПОТОМ":
    -------------------------------------------------
    Оператор += (Конкатенация):
    Процессор берет адрес строки 'welcome_text' в оперативной памяти, 
    выделяет новый блок памяти, копирует туда старый текст и добавляет 
    в конец новый. 
    
    *Важно для твоей батареи (износ 11%):* Такой метод (сначала собрать строку, потом ОДИН раз отправить её через 
    bot.send_message) намного экономнее, чем слать три разных сообщения. 
    Меньше сетевых запросов — дольше живет заряд.

    -------------------------------------------------
    3. КАК МОЖНО БЫЛО СДЕЛАТЬ ЕЩЕ (Альтернатива):
    -------------------------------------------------
    Можно использовать список:
    parts = [base_text]
    if next_lesson: parts.append(lesson_info)
    final_text = "\n".join(parts)
    
    Это считается более профессиональным для очень больших текстов, 
    но для бота += — это самый быстрый и читаемый "атомный" путь.

    -------------------------------------------------
    4. ГДЕ ЕЩЕ ЭТО ПРИМЕНИТЬ:
    -------------------------------------------------
    - В корзине: если (скидка): текст += "Ваша скидка 10%".
    - В играх: если (хп < 20): текст += "Ты почти при смерти!".
    - В погоде: если (дождь): текст += "Возьми зонтик ☔".

    ДЛЯ ЧЕГО ЭТО:
    Для персонализации. Пользователь чувствует, что программа написана 
    специально для него и его текущей ситуации.
    """
...
"""
    ЭТАП ТРАНСМИССИИ И ВИЗУАЛИЗАЦИИ:
    ================================

    ДАННЫЙ БЛОК:
    Превращает внутренние переменные Python в реальное интерактивное 
    сообщение на экране смартфона.

    -------------------------------------------------
    1. ЗАЧЕМ МЫ ЭТО ДЕЛАЕМ (The Goal):
    -------------------------------------------------
    Мы завершаем сеанс логики и переходим к сеансу взаимодействия. 
    Бот выдал всю оперативную сводку и теперь передает инициативу 
    пользователю, подсовывая ему кнопки.

    -------------------------------------------------
    2. ПОЧЕМУ ТАК ЭФФЕКТИВНО (Battery Efficiency):
    -------------------------------------------------
    СУХАЯ ЛОГИКА: 
    Мы пакуем ТЕКСТ и КНОПКИ в один единственный сетевой пакет. 
    Учитывая, что у твоего ноутбука 11% износа батареи, такая минимизация 
    сетевых запросов — это "золотой стандарт". 
    Один запрос = меньше работы Wi-Fi модуля = дольше жизнь заряда.

    -------------------------------------------------
    3. КАК ЭТО РАБОТАЕТ "ПОД КАПОТОМ":
    -------------------------------------------------
    Когда вызывается get_main_keyboard(), программа на долю секунды 
    прыгает в другую функцию, создает там объект ReplyKeyboardMarkup, 
    набивает его кнопками "Сегодня/Завтра" и возвращает обратно 
    в send_message. 

    -------------------------------------------------
    4. ГДЕ ЕЩЕ ЭТО ПРИМЕНИТЬ:
    -------------------------------------------------
    - В любом месте, где нужно закончить диалог и дать выбор.
    - В обработчиках ошибок: "Упс, ошибка. *Выбери действие:* [Повторить] [Отмена]".
    - В финализации заказов: "Ваш чек: 500р. *Выбери действие:* [Оплатить]".

    ДЛЯ ЧЕГО:
    Для создания бесшовного пользовательского опыта. Пользователь видит 
    текст "Выбери действие" и СРАЗУ под ним видит сами кнопки. 
    Глазу не нужно прыгать по экрану.
    """
...
def back_to_menu(message):
    ...
    try:
        ...
        start(message)
"""
    ОБРАБОТЧИК НАВИГАЦИОННОГО ОТКАТА (BACKWARD NAVIGATION):
    ======================================================

    ДАННЫЙ БЛОК:
    Обеспечивает циклическую логику интерфейса. Он связывает второстепенные 
    экраны (расписание, настройки) с Главным Меню.

    -------------------------------------------------
    1. ИДЕНТИФИКАЦИЯ КОНТЕКСТА (try block):
    -------------------------------------------------
    ЛОГИКА: 
    Функция show_main_menu требует объект 'student'. 
    Мы совершаем атомарный запрос к БД, чтобы восстановить контекст 
    пользователя (его класс и настройки).

    -------------------------------------------------
    2. ОБРАБОТКА ИСКЛЮЧЕНИЙ (Exception Handling):
    -------------------------------------------------
    СУХАЯ ЛОГИКА: 
    Если .get() не находит запись, Python выбрасывает ошибку, которая 
    обычно "убивает" программу. Блок 'except' ловит этот взрыв и 
    безопасно перенаправляет пользователя на регистрацию (start). 
    
    *Важно для твоей системы (батарея 11%):* Обработка ошибок в коде 
    предотвращает бесконечные перезапуски бота, которые могли бы 
    потреблять лишние ресурсы процессора при падении скрипта.

    -------------------------------------------------
    3. РЕКУРСИВНЫЙ ВЫЗОВ (show_main_menu):
    -------------------------------------------------
    СВЯЗЬ: 
    Мы не пишем код отрисовки меню заново. Мы вызываем уже готовую 
    функцию, передавая ей "паспорт" студента. Это соблюдение принципа 
    DRY (Don't Repeat Yourself — не повторяйся).

    -------------------------------------------------
    ПРИМЕНЕНИЕ:
    Используется в любом многоуровневом меню: 
    - Банкомат: кнопка "Вернуться к выбору операций".
    - Сайт: ссылка "На главную".
    - Бот: любая навигация назад.
    """
